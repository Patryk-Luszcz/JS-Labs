...
function setChart() {
	dataset = citiesData.map(({ city, temperatures, rgba }) => {
		return {
			label: city,
			data: temperatures,
...
function setStats() {
	var blocks = citiesData.map(({ icon, city, weather, temperatures }) => {
		return `<div style="margin: 20px">
            <img class="icon" src="${icon}" height="75" width="75">
            <p id="iconWeather">${city}: ${weather} ${temperatures[0]}C</p>
        </div>`;
	});
